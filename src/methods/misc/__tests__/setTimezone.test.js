...
test(`should do nothing without 'moment-timezone'`, function () {
...
test(`should set the 'timezone' that 'moment' will use`, async function () {
...
test(`should have an accessible 'currentIANAZoneName'`, async function () {
  await import('moment-timezone');
  const extended = decorate(moment);
...
test(`should throw an error if you try to set an invalid 'timezone'`, async function () {
  await import('moment-timezone');
  const extended = decorate(moment);
