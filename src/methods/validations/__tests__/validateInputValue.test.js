...
test(`should throw an error if you do not pass a 'format'`, function () {
  expect(() => extended.validateInputValue('12/02/2022')).toThrow(
    `[moment.validateInputValue] You did not supply a 'format'`
  );
});

test(`should return 'undefined' if you pass a falsy 'value'`, function () {
...
test(`should return a 'moment' if you pass a 'value' that can stricly be parsed by your supplied 'format'`, function () {
  let temp = extended.validateInputValue('2022-02-22', 'YYYY-MM-DD');
  expect(extended.isMoment(temp)).toBeTruthy();
});

test(`should return 'undefined' if you pass a 'value' that can not be stricly be parsed by your supplied 'format'`, function () {
  let temp = extended.validateInputValue('2022-02', 'MM/DD/YYYY');
  expect(temp).toBeUndefined();
});

test(`should remove 'maskChar' from the 'input', but return 'undefined' for being incomplete`, function () {
